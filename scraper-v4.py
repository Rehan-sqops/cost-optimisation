...
# ------------ Setup Cronjob (runs every 1 min) -------------
def setup_cron():
...
if __name__ == '__main__':
    ...
    try:
        ...
        with open(os.path.expanduser("~/cadvisor_cron.txt"), "a") as f:
            f.write(f"\n📊 [{now}] LIVE RESOURCE USAGE\n")
            f.write(f"{'Node':<28} {'Namespace':<12} {'Owner':<12} {'Pod':<30} {'Container':<22} {'CPU (cores/sec)':>17} {'MEM (MiB)':>12} {'CPU Req':>8} {'CPU Lim':>8} {'Mem Req':>9} {'Mem Lim':>9}\n")
            f.write("-" * 185 + "\n")
            ...
            for key in sorted(curr_metrics):
                ...
                f.write(f"{node:<28} {ns:<12} {owner:<12} {pod:<30} {container:<22} {cpu:>17.4f} {mem:>12.2f} {cpu_req:>8.2f} {cpu_lim:>8.2f} {mem_req:>9.2f} {mem_lim:>9.2f}\n")
